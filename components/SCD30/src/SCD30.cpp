...
#define POWERSWITCHPIN	GPIO_NUM_25 // to switch power to Sensor off when not responding
									// VDD connected to NPN transistor
...
esp_err_t SCD30::setAltitudeCompensation(uint16_t altitude) {
...
esp_err_t SCD30::getSettingValue(uint16_t registerAddress, uint16_t *val) {
	...
	do {
		...
		err = i2c_master_write_to_device(_i2cPort, SCD30_ADDRESS, (uint8_t *)&registerAddress, sizeof(registerAddress), I2C_TIMEOUT_MS / portTICK_PERIOD_MS);
		if (err != ESP_OK) {
...
esp_err_t SCD30::readRegister(uint16_t registerAddress , uint16_t *data ) {
	...
	if ( err == ESP_OK) {
		vTaskDelay(3 / portTICK_PERIOD_MS);
		err = i2c_master_read_from_device(_i2cPort, SCD30_ADDRESS, (uint8_t *) &receivedBytes, sizeof(receivedBytes), I2C_TIMEOUT_MS / portTICK_PERIOD_MS);
		if ( err == ESP_OK) {
